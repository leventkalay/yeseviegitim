...
export class KurumComponent implements OnInit {
  ...
  trackId(index: number, item: IKurum): number {
    return item.id!;
  }

  delete(kurum: IKurum): void {
    const modalRef = this.modalService.open(KurumDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IKurum[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
